...
// you can also set the desc `titlesGroup` or `attributedTitlesGroup`, pick one of two, `attributedTitlesGroup` first
    ...
    public func setImagesGroup(_ imagesGroup: Array<UIImage?>,
                               titlesGroup: [String?]? = nil,
                               attributedTitlesGroup: [NSAttributedString?]? = nil) {
...
// MARK: - PageControl setting
    ...
    public var pageControlIndictirColor = UIColor.gray {
        didSet {
            pageControl.pageIndicatorTintColor = pageControlIndictirColor
        }
    }
    /// Current dot color, the default `white`
    public var pageControlCurrentIndictirColor = UIColor.white {
        didSet {
            pageControl.currentPageIndicatorTintColor = pageControlCurrentIndictirColor
        }
    }
    /// The current dot image
    public var pageControlCurrentIndictorImage: UIImage? {
        didSet {
            pageControl.currentItemImage = pageControlCurrentIndictorImage
        }
    }
    /// The dot image
    public var pageControlIndictorImage: UIImage? {
        didSet {
            pageControl.itemImage = pageControlIndictorImage
        }
    }
    ...
    public var pageControlHeight: CGFloat = 25 {
        didSet {
            pageControl.frame = CGRect(x: 0,
                                       y: frame.size.height-pageControlHeight,
    ...
    public var pageControlItemSize = CGSize(width: 6, height: 6) {
        didSet {
            pageControl.controlSize = pageControlItemSize
        }
    }
    /// The size of current dot
    public var pageControlCurrentItemSize: CGSize? {
        didSet {
            pageControl.currentControlSize = pageControlCurrentItemSize
        }
    }
    /// The space of dot
    public var pageControlSpacing: CGFloat = 6 {
        didSet {
            pageControl.controlSpacing = pageControlSpacing
        }
    }
    /// pageControl Alignment, left/right/center , default `center`
    public var pageControlAlignment: ZCyclePageControlAlignment = .center {
        didSet {
            pageControl.alignment = pageControlAlignment
        }
    }
    /// the radius of dot
    public var pageControlItemRadius: CGFloat? {
        didSet {
            pageControl.itemCornerRadius = pageControlItemRadius
        }
    }
    /// the radius of current dot
    public var pageControlCurrentItemRadius: CGFloat? {
        didSet {
            pageControl.currentItemCornerRadius = pageControlCurrentItemRadius
        }
    }
...
// Click and scroll events are in the form of closures
    ...
    private func addPageControl() {
        ...
        pageControl = ZCyclePageControl(frame: CGRect(x: 0,
                                                      y: bounds.size.height - pageControlHeight,
...
extension ZCycleView {
    ...
    fileprivate func startTimer() {
        ...
        cancelTimer()
        timer = Timer.init(timeInterval: Double(timeInterval),
                           target: self,
